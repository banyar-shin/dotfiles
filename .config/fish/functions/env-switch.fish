...
function auto_env_switch --on-variable PWD
    ...
    # Check if we're still within the environment's root directory
    if test -n "$LAST_ENV_DIR"
        # Only deactivate if we've moved completely outside the environment's directory tree
        if not string match -q "$LAST_ENV_DIR*" "$CURR_DIR"
            set deactivated 0

            # Check if we need to deactivate a virtualenv
            if test -d "$LAST_ENV_DIR/.venv"
    ...
    # Only check for new environments if we're not in any environment
    if test -z "$LAST_ENV_DIR"
